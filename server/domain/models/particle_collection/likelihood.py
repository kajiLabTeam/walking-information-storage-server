...
class Likelihood:
    def __init__(self, particles: List[Particle]):
        ...
        self.__mean_x = sum(particle.get_x() for particle in self.__particles) / len(
            self.__particles
        )
        self.__mean_y = sum(particle.get_y() for particle in self.__particles) / len(
            self.__particles
        )
    ...
    def get_std_dev_x(self) -> float:
        return self.__std_dev_x

    def get_std_dev_y(self) -> float:
        return self.__std_dev_y

    def calculate_normalized_probability_densities(self) -> List[float]:
    ...
    def __calculate_probability_density(self, x: int, y: int) -> float:
