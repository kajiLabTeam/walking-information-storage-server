...
class RatioWaveFingerprint:
    ...
    def calculate_match_rate(self, fp_model_df: pd.DataFrame, ratio_wave_df: pd.DataFrame) -> float:
        fp_model_mac_addresses = set(fp_model_df["mac_address"])
        ratio_wave_mac_addresses = set(ratio_wave_df["mac_address"])

        intersection_count = len(fp_model_mac_addresses.intersection(ratio_wave_mac_addresses))
